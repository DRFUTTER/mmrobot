...
*/
  ...
  /*---------------*/
  new_port_settings.c_cc[VMIN] = 3;      /* block until n bytes are received */
  new_port_settings.c_cc[VTIME] = 5;     /* block until a timer expires (n * 100 mSec.) */
...
int RS232_availableBytes(int comport_number)
{
  int nBytes = 0;
  ioctl(Cport[comport_number],FIONREAD,&nBytes);
  return nBytes;
}

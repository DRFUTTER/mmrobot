...
int addrSleepTime = 1000 * 1000;  //1s
int initSleepTime = 1000 * 1000;  //100ms to wait for data
...
int SmartMotor::CheckMotorsAddress()
{
    ...
    int checkCount = 0;
    while (RS232_availableBytes(cport_nr) < len + (SmartMotor::count)*2) //Read: RADDR (address\r) there are two characters per address
    {
        usleep(100);
        checkCount++;
        if (checkCount > 30)
    ...
    else
    {
        ...
        std::string addrStr = "RADDR ";
        for (int i = 1; i <= SmartMotor::count; i++)
        {
            addrStr.append(1, i + 48);
            addrStr.append(1, '\r');
        }
        std::cout << "Received string address: " << addrStr << std::endl;
...
//Send the velocity
int SmartMotor::sendVelocity(int revPerSec)
{
